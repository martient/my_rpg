...
** get_next_line
** File description:
** Functions that read a file line by line
*/
...
char *get_next_line(int fd)
{
	...
	if (!buff)
		buff = reset_buff(NULL, sizeof(char) * READ_SIZE_H_ + 1, 1);
	if (!my_strlen(buff, 1))
...
char *fill_the_buff(int fd, char *buff)
{
	char *tmp = reset_buff(NULL, sizeof(char) *
			(READ_SIZE_H_ + my_strlen(buff, 0) + 1), 1);

	if (!tmp)
		return NULL;
	tmp = my_strcat(tmp, buff, 0);
	buff = reset_buff(buff, sizeof(char) * READ_SIZE_H_ + 1, 1);
	if (!buff)
		return NULL;
	if (read(fd, buff, READ_SIZE_H_) <= 0) {
		free(buff);
		if (*tmp != '\0')
			return tmp;
		free(tmp);
